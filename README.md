# Academic Paper Checker / 学術論文チェック

学術論文の投稿前に、原稿の構成・表現・方法・統計・引用文献・新規性をAIがチェックし、改善提案を生成するデスクトップアプリケーションです。
...
- 行番号付きまたは段落・文番号付きの修正提案を Markdown / テキスト / Word 形式で出力します

本ツールは、投稿前に著者自身が原稿の品質をセルフチェックするための補助ツールです。生成される指摘や改善提案は参考情報であり、最終的な判断と修正は著者が行ってください。

## 現在の状態

**v0.5.0** — 原稿チェックパイプラインが実装済み。以下の機能が利用可能です：
...
スタートメニューまたはデスクトップの **Academic Paper Checker** を起動してください。
...
1. チェックしたい docx 原稿を準備する（PDF は任意）
2. Academic Paper Checker を起動する
...
- **原稿は機密情報です。** 外部 API へ送信する場合は、当該ジャーナル・出版社・所属機関の AI 利用ポリシーに従ってください。
