...
- When discussing analytical methods, also examine whether the current method is fundamentally appropriate for the research question, not just how to improve it.`;
  ...
  const buildAndCopyExternalPrompt = async (checkName: string) => {
    ...
    try {
      ...
      let prompt = `You are a peer reviewer for a scientific journal. Below is a list of review comments generated by an AI review tool. For each comment, judge whether the criticism is valid.
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 6, fontSize: "11px", color: "#888" }}>
              {(() => {
                const doneCount = reviewers.filter(s => results[s.name] === "done").length;
                const total = reviewers.length;
                if (total === 0) return "レビュワー未設定";
                return `レビュワー状況: ${doneCount}/${total} 完了`;
              })()}
              {mergeDone && " · 統合済"}
            </div>
            {reviewers.map((slot) => {
      ...
      <div style={{ display: "flex", gap: 0, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: "0 0 420px", overflowY: "auto", paddingRight: 4 }}>
          ...
          <div className="next-step">
            ...
            {(structureMergeDone && expressionMergeDone && methodsStatsMergeDone && logicArgumentMergeDone) && !finalMergeDone && "次: 最終査読コメントを生成してください"}
            {finalMergeDone && "査読完了。「査読結果作成」メニューで出力を確認してください"}
          </div>
