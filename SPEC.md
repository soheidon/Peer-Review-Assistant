...
# Academic Paper Checker / 学術論文チェック 全体仕様書

## 0. この仕様書の位置づけ

本仕様書は、**Academic Paper Checker / 学術論文チェック** の全体仕様を定義する。

本ツールは、学術論文の投稿前に著者が原稿の品質をセルフチェックするためのデスクトップアプリケーションである。docx 原稿と、Word から手作業で出力した行番号付き PDF を入力し、原稿本文、セクション、行番号、段落番号、文番号、引用文献、本文中引用を前処理したうえで、複数 LLM、文献データベース、手入力結果を統合し、チェックレポートを Markdown 形式で出力する。
...
Academic Paper Checker は、学術論文の投稿前に、構成、表現、方法・統計、引用文献、先行研究との関係を点検し、行番号付きまたは段落・文番号付きの改善提案を生成するツールである。

本ツールは投稿前のセルフチェックを支援する補助ツールであり、生成される指摘や改善提案は参考情報である。最終的な判断と修正は著者が行う。
...
新規性チェックは統括AI（summary）の Pro/reasoning モデルを使用する。Deep Research 統合結果は原稿チェック時の参照データとして利用する。ジャーナルプロファイルの `publication_criteria`（技術的健全性重視/新規性重視 etc.）に基づき評価軸を自動調整する。
### 7.2 LLM スロット

| スロット | 役割 |
|---|---|
| まとめLLM (summary) | マージ処理、最終チェックレポート整形 |
...
第2層：各チェック項目内でのマージ・チェックレポート候補作成
第3層：全体マージ・最終チェックレポート作成
```

### 8.2 チェック項目内マージ

LLM01/02/03 と手入力結果を統合し、チェック項目ごとのチェックレポート候補を作成する。
...
全チェック項目の統合結果、原稿要約、line_map、paragraph_sentence_map を統合し、最終チェックレポートを生成する。出力形式（Markdown / DOCX / TXT）と言語（英語 / 日本語）を選択可能。
...
[Ready for Submission / Ready with Minor Changes / Needs Revision Before Submission / Major Rework Recommended]
...
## 6. Citation and Literature Concerns

## 7. Originality and Overlap
...
Markdown から python-docx で変換。見出し、太字、箇条書き、罫線等の書式を保持する。チェックレポートを Word ファイルとして出力する場合に使用。
...
**Key Issue（重要な修正事項）**: 研究目的と方法の不一致、再現不能な方法説明、統計手法の誤り、結果と結論の不一致、主要先行研究の欠落、重大な倫理的懸念

**Minor Suggestion（軽微な改善提案）**: 英語表現の軽微な不自然さ、語句の曖昧さ、表記揺れ、図表タイトル修正、引用形式の不備
...
│ 前処理   │                                      │
│ 文献確認 │ Active Panel                         │
│ 原稿チェック│ (home/project/preprocess/          │
...
```
本ツールは投稿前のセルフチェックを支援するツールであり、生成される指摘は参考情報です。最終的な判断と修正は著者が行ってください。
原稿を外部 API へ送信する場合は、当該ジャーナル・出版社・所属機関の AI 利用ポリシーに従ってください。
...
| **GUI全般** | |
| 全パネル（ホーム/プロジェクト/前処理/文献確認/ジャーナル/新規性/原稿チェック/結果/設定） | ✅ |
| 全ボタン日本語ラベル | ✅ |
...
3. **LLM出力の矛盾は多数決で消さない** → conflict として保持し、著者の判断に委ねる
